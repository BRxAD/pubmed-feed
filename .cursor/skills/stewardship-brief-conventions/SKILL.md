...
- **In the news** — WHO / CIDRAP / Google News RSS polled daily (`/api/cron/news-rss`). Google News query includes antimicrobial stewardship, antibiotic stewardship, **antimicrobial resistance**, and antibiotic resistance. Items land as **pending**; editors approve on **`/feed`** (after secret unlock). Homepage: steel panel floats left, tools float right; lead uses a **compact side thumb + headline** so it sits between them; “Also…” is normal flow (not multicol) so stories wrap under the news to the left edge. Optional article thumbnails (`image_url`). Scripts: `scripts/add_news_items.sql`, `scripts/add_news_items_image_url.sql`.
