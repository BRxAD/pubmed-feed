...
- **Causality:** Causal verbs **only** for **RCTs**, or **systematic reviews / meta-analyses clearly limited to RCT evidence**. Mixed SR/MA (observational + RCT) stay non-causal. Observational / cohort / cross-sectional / interrupted time series / pre-post / quasi-experimental / target-trial emulation → associations or patterns only (“associated with”, “tied to”, “use was lower”). When unsure, default non-causal. Enforced on **headlines and on RESULTS / BOTTOM LINE** (shared check in `lib/brief/causality.ts`); METHODS may describe an intervention’s aim. New ingest only unless asked to rewrite old rows.
...
- Headline + bottom-line prompts: ID/AMS experts, stewardship angle, RCT or RCT-only SR for causal language (enforced on headlines **and** RESULTS/BOTTOM LINE), do not over-promise vs sensitivity analyses, headline this paper not cited literature.
...
- [ ] Summaries/headlines: ID/AMS audience, stewardship angle, causal verbs only for RCT or RCT-only SR (headlines **and** RESULTS/BOTTOM LINE), no over-promise, named outcome (not bare “rates”), this paper not cited literature
