...
- **In the news** — WHO / CIDRAP / Google News RSS polled daily (`/api/cron/news-rss`). Google News query includes antimicrobial stewardship, antibiotic stewardship, **antimicrobial resistance**, and antibiotic resistance. Items land as **pending**; editors approve on **`/feed`** (after secret unlock). Homepage: solid steel (`#2A79A7`) panel **floats left of the lead** (not sticky); Save / Top 10 / Digest float right; lead + “Also…” wrap between them and **expand under the news** so the left gutter is not wasted. Optional article thumbnails (`image_url`). Scripts: `scripts/add_news_items.sql`, `scripts/add_news_items_image_url.sql`.
