...
- **In the news** — WHO / CIDRAP / Google News RSS polled daily (`/api/cron/news-rss`). Google News query includes antimicrobial stewardship, antibiotic stewardship, **antimicrobial resistance**, and antibiotic resistance. Items need a real **http(s)** link to be stored, approved, or shown. Editors approve on **`/feed`**. Homepage: 3-col top (news | lead+Also single-file | tools); Also continues in a **full-width 2-col** once past both side panels. Lead uses a compact side thumb. Scripts: `scripts/add_news_items.sql`, `scripts/add_news_items_image_url.sql`.
