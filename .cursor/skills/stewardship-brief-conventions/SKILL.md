...
- **Graphic takeaway 2.0 (live):** plum gradient over the assigned story image (`#1C0B19`, **95%** opaque through the top **40%**, then down to **75%** at the bottom). 16:9 PNG. Headline + boxes occupy the **left 75%**; right side stays open. Journal (BookOpen, sky) at top left, no chip. Inverted/light logo at top (logo 15% larger than the prior 42px mark). **Key takeaway** (`bottomLine`) with Sparkles icon. **Methods** (Users) left / **Study findings** (`results`, BarChart3) right, pale-sky labels. Footer citation: first author, original article title, then journal + year. QR tab flush off the right, same salmon wash + salmon type as Key takeaway. Salmon chip still opens the download/share popup. **1.0** was the 4:5 navy left-shade photo card; keep it in git if we restore or offer it as an option later.
